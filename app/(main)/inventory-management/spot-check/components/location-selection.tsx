...
import { mockLocations, getLocationsByDepartment } from '@/lib/mock/inventory-data';
...
export function LocationSelection({ formData, setFormData, onNext, onBack }: LocationSelectionProps) {
  ...
  console.log('Department Locations:', departmentLocations);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Choose the locations you want to include in this spot check count.
        </p>
      </div>
        ...
        {filteredLocations.map((location) => (
          ...
          >
            ...
            <div className="mt-2 text-sm text-muted-foreground">
              <div>Items: {location.itemCount}</div>
              <div>Last Count: {location.lastCount}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="flex justify-between pt-6">
        ...
        <Button
          onClick={onNext}
          disabled={formData.selectedLocations.length === 0}
        >
          Next
        </Button>
      </div>
