...
import { WorkflowAdvancedFilter } from './workflow-filter-builder'
import { FilterType } from '@/lib/utils/filter-storage'
import { WorkflowQuickFilters, QuickFilterOption } from './workflow-quick-filters'

type WorkflowFilter = FilterType<Workflow> & { id: string }
...
export function WorkflowDataTable<TData, TValue>({
  ...
  const [appliedFilters, setAppliedFilters] = React.useState<WorkflowFilter[]>([])
      ...
      result = result.filter((item: any) => {
        switch (quickFilter.type) {
          case 'status':
            ...
            return item.status === quickFilter.value
          case 'type':
            if (quickFilter.value === 'all-types') return true
            return item.type === quickFilter.value
          case 'recent':
            if (quickFilter.value === 'all-recent') return true
            const daysDiff = Math.floor((new Date().getTime() - new Date(item.lastModified).getTime()) / (1000 * 60 * 60 * 24))
            if (quickFilter.value === 'last-week') return daysDiff <= 7
            if (quickFilter.value === 'last-month') return daysDiff <= 30
            if (quickFilter.value === 'last-quarter') return daysDiff <= 90
            return true
          default:
            return true
        }
  ...
  const handleApplyFilters = (filters: WorkflowFilter[]) => {
