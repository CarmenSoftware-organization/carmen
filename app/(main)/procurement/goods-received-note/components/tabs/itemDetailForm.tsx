...
interface ItemDetailFormProps {
    ...
    handleItemChange?: (id: string, field: keyof GoodsReceiveNoteItem, value: string | number | boolean) => void;
    onClose: () => void;
    onSave: (item: GoodsReceiveNoteItem) => void;
}

export default function ItemDetailForm({ item: initialItem, mode: initialMode, handleItemChange, onClose, onSave }: ItemDetailFormProps) {
    const [mode, setMode] = useState<GoodsReceiveNoteMode | 'add'>(initialMode);
    const [item, setItem] = useState<GoodsReceiveNoteItem>(initialItem || {
    ...
    const handleChange = (field: keyof GoodsReceiveNoteItem, value: string | number | boolean) => {
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        <div className="grid grid-cols-6 gap-4">
                            ...
                            <div className="col-span-1">
                                <Label htmlFor={`purchaseOrderRef-${item.id}`}>Job code</Label>
                                <Input
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-2">
                            <h3 className="text-base font-semibold">Quantity and Delivery</h3>
                            <div className="flex space-x-2">
                                <Button variant="outline" size="sm">On Hand</Button>
                                <Button variant="outline" size="sm">On Order</Button>
                                <Button variant="outline" size="sm">Vendor Comparison</Button>
                            </div>
                        </div>
                        <div className="grid grid-cols-6 gap-4">
                            <div>
                                ...
                                <Input
                                    ...
                                    readOnly={mode === "view"}
                                />
                                <div className="text-sm text-gray-500 mt-1">Base: Kg | 1 Bag = 0.5 Kg</div>
                            </div>
                            <div>
                                <Label htmlFor={`orderedQuantity-${item.id}`}>Order Quantity</Label>
                                <Input
                                    id={`orderedQuantity-${item.id}`}
                                    type="number"
                                    value={item.orderedQuantity || ""}
                                    onChange={(e) => handleChange("orderedQuantity", parseFloat(e.target.value))}
                                    readOnly={mode === "view"}
                                />
                                <div className="text-xs text-gray-500 mt-1">
                                    {(item.orderedQuantity * item.conversionRate).toFixed(2)} {item.baseUnit}
                                </div>
                            </div>
                            <div>
                                <Label htmlFor={`receivedQuantity-${item.id}`}>Receiving Quantity</Label>
                                <Input
                                    id={`receivedQuantity-${item.id}`}
                                    type="number"
                                    value={item.receivedQuantity || ""}
                                    onChange={(e) => handleChange("receivedQuantity", parseFloat(e.target.value))}
                                    readOnly={mode === "view"}
                                />
                                <div className="text-xs text-gray-500 mt-1">
                            ...
                            <div>
                                ...
                                <Input
                                    ...
                                    onChange={(e) => handleChange("isFreeOfCharge", e.target.checked)}
                                    readOnly={mode === "view"}
                                    className="w-4 h-4"
                                />
                                </div>
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    <Input
                                        ...
                                        type="date"
                                        value={item.deliveryDate ? item.deliveryDate.toISOString().split('T')[0] : ""}
                            ...
                            <div>
                                ...
                                <Input
                                    ...
                                    onChange={(e) => handleChange("deliveryPoint", e.target.value)}
                                    readOnly={mode === "view"}
                                />
                            </div>
                        </div>
                        <div className="grid grid-cols-3 gap-4 mt-2 bg-gray-100 p-2 text-sm">
                            ...
                            <div>
                                ...
                                <div className="text-sm">{item.inventoryReorderThreshold} Kg</div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <div className="grid grid-cols-4 gap-4">
                                ...
                                <div>
                                    <Label htmlFor={`exchangeRate-${item.id}`}>Exch. Rate</Label>
                                    <Input
                                ...
                                <div>
                                    ...
                                    <Input
                                        ...
                                        onChange={(e) => handleChange("baseUnitPrice", parseFloat(e.target.value))}
                                        readOnly={mode === "view"}
                                        className="text-right"
                                    />
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-4 mt-4">
                                <div>
                                    <Label htmlFor={`discountAdjustment-${item.id}`}>Adj. Disc. Rate (%)</Label>
                                    <div className="flex items-center space-x-2">
                                        <Checkbox
                                            id={`discountAdjustment-${item.id}`}
                                            checked={item.adjustments?.discount}
                                            onCheckedChange={(checked) => handleChange("adjustments", checked )}
                                            disabled={mode === "view"}
                                        />
                                        <Input
                                ...
                                <div>
                                    <Label htmlFor={`overrideDiscount-${item.id}`}>Override Discount Amount</Label>
                                    <Input
                            ...
                            <div className="grid grid-cols-2 gap-4 mt-4">
                                <div>
                                    <Label htmlFor={`taxAdjustment-${item.id}`}>Adj. Tax Rate (%)</Label>
                                    <div className="flex items-center space-x-2">
                                        <Checkbox
                                ...
                                <div>
                                    <Label htmlFor={`taxAmount-${item.id}`}>Override Tax Amount</Label>
                                    <Input
                            ...
                            <div className="grid grid-cols-3 gap-4 mt-4 bg-gray-100 p-2 text-sm text-muted-foreground">
                                <div>
                                    ...
                                    <div className="text-sm">{item.lastPurchasePrice?.toFixed(2) || '0.00'} per Kg</div>
                                </div>
                                <div>
                        ...
                        <div>
                            <h3 className="text-base font-semibold mb-2">Calculated Amounts</h3>
                            <div className="space-y-2">
                                <div className="grid grid-cols-3 gap-4 font-semibold text-muted-foreground mb-2 text-sm">
                                    <div>Description</div>
                                    <div className="text-right">Total Amount ({item.currency})</div>
                                    <div className="text-right text-xs">Base Amount ({item.baseCurrency})</div>
                                </div>
                                {[
                                    { label: "Subtotal Amount", total: item.subTotalAmount, base: item.baseSubTotalAmount },
                                    { label: "Discount Amount", total: item.discountAmount, base: item.baseDiscountAmount },
                                    { label: "Net Amount", total: item.netAmount, base: item.baseNetAmount },
                                    { label: "Tax Amount", total: item.taxAmount, base: item.baseTaxAmount },
                                    { label: "Total Amount", total: item.totalAmount, base: item.baseTotalAmount },
                                ].map((row, index) => (
                                    <React.Fragment key={row.label}>
                                        <div className="grid grid-cols-3 gap-4 text-sm">
                                            <div>{row.label}</div>
                                            <div className="text-right">{row.total?.toFixed(2) || '0.00'}</div>
                                            <div className="text-right text-muted-foreground text-xs">{row.base?.toFixed(2) || '0.00'}</div>
                                        </div>
