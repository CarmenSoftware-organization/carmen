...
interface PRAdvancedFilterProps {
  onApplyFilters: (filters: FilterType<PurchaseRequest>[]) => void
  onClearFilters: () => void
...
const filterFields = [
  ...
  { value: 'currentWorkflowStage', label: 'Stage' },
...
export function PRAdvancedFilter({ onApplyFilters, onClearFilters, activeFiltersCount }: PRAdvancedFilterProps) {
  ...
  const loadSavedFilters = () => {
    try {
      const filters = getSavedFilters<PurchaseRequest>()
  ...
  const handleDeleteSavedFilter = async (filterId: string) => {
    try {
      await deleteFilter<PurchaseRequest>(filterId)
  ...
  const handleToggleStar = async (filter: SavedFilter<PurchaseRequest>) => {
    try {
      const updatedFilter = await saveFilter<PurchaseRequest>({
        ...filter,
  ...
  const addFilter = () => {
    setActiveFilters([...activeFilters, {
      field: 'refNumber' as keyof PurchaseRequest,
      operator: 'equals',
      value: '',
    }])
  }
  ...
  const handleFieldChange = (index: number, value: keyof PurchaseRequest) => {
    const newFilters = [...activeFilters]
    if (newFilters[index]) {
  ...
  const getFilterDisplay = (filter: FilterType<PurchaseRequest>): string => {
                      ...
                      <div className="grid grid-cols-12 gap-2">
                        <div className="col-span-5">
                          <Select
                            ...
                            onValueChange={(value) => handleFieldChange(index, value as keyof PurchaseRequest)}
                          >
                            <SelectTrigger>
