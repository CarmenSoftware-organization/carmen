...
// Content-only component for use inside existing Dialog
export function InventoryBreakdownContent({ itemData }: { itemData: { name: string; description: string; status: string } }) {
  return (
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-6xl max-h-[90vh] overflow-y-auto [&>button]:hidden">
        ...
        <InventoryBreakdownContent itemData={itemData} />
      </DialogContent>
    </Dialog>
  )
}