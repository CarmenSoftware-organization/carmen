...
flowchart TD
    ...
    SelectLoc --> EnterMin[User enters Min Stock Level]
    EnterMin --> EnterMax[User enters Max Stock Level]
    EnterMax --> EnterReorder[User enters Reorder Point<br/>Optional]
    EnterReorder --> EnterQty[User enters Reorder Quantity<br/>Optional]
    EnterQty --> ClickSave[User clicks Save]
    ...
    CheckDup -->|New| ValidateReorder{Validate<br/>Reorder Point}
    ValidateReorder -->|Outside Range| WarnReorder[Show Warning:<br/>Reorder point should be<br/>between min and max]
    WarnReorder --> ConfirmProceed{User confirms<br/>proceed?}
    ConfirmProceed -->|No| ShowForm
    ConfirmProceed -->|Yes| BeginTx

    ValidateReorder -->|Valid or Empty| BeginTx[Begin Database Transaction]
    BeginTx --> InsertAssign[Insert into product_locations<br/>product_id, location_id<br/>min, max, reorder_point/qty]
