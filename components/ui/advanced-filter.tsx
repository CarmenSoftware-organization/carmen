...
export function AdvancedFilter<T>({ onApplyFilters, onClearFilters, filterFields = [] }: AdvancedFilterProps<T>) {
  ...
  const loadSavedFilters = async () => {
    try {
      const filters = await getSavedFilters<T>()
  ...
  const handleToggleStar = async (filter: SavedFilter<T>) => {
    try {
      await saveFilter<T>({
        ...filter,
  ...
  const handleFieldChange = (index: number, value: keyof T) => {
    const newFilters = [...activeFilters]
    if (newFilters[index]) {
  ...
  const getFilterDisplay = (filter: FilterType<T>): string => {
    try {
                      ...
                      <div className="grid grid-cols-12 gap-2">
                        <div className="col-span-5">
                          <Select
                            ...
                            onValueChange={(value) => handleFieldChange(index, value as keyof T)}
                          >
                            <SelectTrigger>
