...
const purchaseOrders = [
  { id: 1, number: 'PO-001', date: '2023-08-01', vendor: 'Acme Inc.', total: 1000.00, status: 'Open', email: 'vendor@acme.com' },
  { id: 2, number: 'PO-002', date: '2023-08-02', vendor: 'XYZ Corp', total: 1500.50, status: 'Send', email: 'vendor@xyz.com' },
  { id: 3, number: 'PO-003', date: '2023-08-03', vendor: 'Tech Solutions', total: 2500.75, status: 'Closed', email: 'vendor@techsolutions.com' },
  { id: 4, number: 'PO-004', date: '2023-08-04', vendor: 'Global Supplies', total: 750.25, status: 'Open', email: 'vendor@globalsupplies.com' },
  { id: 5, number: 'PO-005', date: '2023-08-05', vendor: 'Best Electronics', total: 3000.00, status: 'Send', email: 'vendor@bestelectronics.com' },
  { id: 6, number: 'PO-006', date: '2023-08-06', vendor: 'Office Depot', total: 500.50, status: 'Partial Received', email: 'vendor@officedepot.com' },
  { id: 7, number: 'PO-007', date: '2023-08-07', vendor: 'Furniture Co.', total: 4500.00, status: 'Open', email: 'vendor@furnitureco.com' },
  { id: 8, number: 'PO-008', date: '2023-08-08', vendor: 'Industrial Supplies', total: 2000.25, status: 'Send', email: 'vendor@industrialsupplies.com' },
  { id: 9, number: 'PO-009', date: '2023-08-09', vendor: 'Tech Innovations', total: 1750.75, status: 'Closed', email: 'vendor@techinnovations.com' },
  { id: 10, number: 'PO-010', date: '2023-08-10', vendor: 'Global Traders', total: 3500.00, status: 'Partial Received', email: 'vendor@globaltraders.com' }
]

// Mock data for Purchase Requests
const purchaseRequests = [
  { id: 1, refNumber: 'PR-001', date: '2023-08-01', description: 'Office Supplies', deliveryDate: '2023-08-15', status: 'Approved' },
  { id: 2, refNumber: 'PR-002', date: '2023-08-02', description: 'IT Equipment', deliveryDate: '2023-08-20', status: 'Approved' },
...
const PurchaseOrderList: React.FC = () => {
  ...
  const handleBulkAction = (action: 'delete' | 'void' | 'close', poIds: number[] = selectedPOs) => {
      ...
      setFilteredPOs(prevPOs => 
        prevPOs.map(po => 
          poIds.includes(po.id) 
            ? {...po, status: action === 'delete' ? 'Deleted' : action === 'void' ? 'Voided' : 'Closed'} 
            : po
        )
      );
    }
    ...
    <div>
      <div className="flex justify-between items-center mb-4">
        <div className="flex space-x-2">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="w-[180px]">
              ...
              <DropdownMenuItem onSelect={() => setStatusFilter("Send")}>Send</DropdownMenuItem>
        ...
        <div className="flex space-x-2">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" disabled={selectedPOs.length === 0}>
                Batch Actions
              </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent>
              ...
              {selectedPOs.length > 0 && (
                <>
                  {selectedPOs.some(id => currentItems.find(po => po.id === id)?.status === 'Open') && (
                    <DropdownMenuItem onSelect={() => handleBulkAction('delete')}>Delete Selected</DropdownMenuItem>
                  )}
                  {selectedPOs.some(id => ['Send', 'Partial Received'].includes(currentItems.find(po => po.id === id)?.status || '')) && (
                    <DropdownMenuItem onSelect={() => handleBulkAction('void')}>Void Selected</DropdownMenuItem>
                  )}
                  {selectedPOs.some(id => ['Open', 'Send', 'Partial Received'].includes(currentItems.find(po => po.id === id)?.status || '')) && (
                    <DropdownMenuItem onSelect={() => handleBulkAction('close')}>Close Selected</DropdownMenuItem>
                  )}
                </>
              )}
            </DropdownMenuContent>
      ...
      <Dialog open={isCreateFromPROpen} onOpenChange={setIsCreateFromPROpen}>
        <DialogContent className="sm:max-w-[800px]">
          ...
          <DialogFooter>
            <Button onClick={handleGeneratePOs} disabled={selectedPRs.length === 0}>Generate PO</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={isCreateManuallyOpen} onOpenChange={setIsCreateManuallyOpen}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="po-number" className="text-right">PO Number</Label>
              <Input
                id="po-number"
                value={newManualPO.number}
                onChange={(e) => setNewManualPO({...newManualPO, number: e.target.value})}
                className="col-span-3"
              />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="po-date" className="text-right">Date</Label>
              <Input
                id="po-date"
                type="date"
                value={newManualPO.date}
                onChange={(e) => setNewManualPO({...newManualPO, date: e.target.value})}
                className="col-span-3"
              />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="po-vendor" className="text-right">Vendor</Label>
              <Input
                id="po-vendor"
                value={newManualPO.vendor}
                onChange={(e) => setNewManualPO({...newManualPO, vendor: e.target.value})}
                className="col-span-3"
              />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="po-total" className="text-right">Total</Label>
              <Input
                id="po-total"
                type="number"
                value={newManualPO.total}
                onChange={(e) => setNewManualPO({...newManualPO, total: parseFloat(e.target.value)})}
                className="col-span-3"
              />
            </div>
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="po-email" className="text-right">Email</Label>
              <Input
                id="po-email"
                type="email"
                value={newManualPO.email}
                onChange={(e) => setNewManualPO({...newManualPO, email: e.target.value})}
                className="col-span-3"
              />
            </div>
          </div>
