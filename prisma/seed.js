...
async function loadSeedData() {
  ...
  const resourceDefinitions = JSON.parse(
    fs.readFileSync(path.join(seedDataPath, 'resource-definitions.json'), 'utf8')
  )
  
  const environmentDefinitions = JSON.parse(
    fs.readFileSync(path.join(seedDataPath, 'environment-definitions.json'), 'utf8')
  )
  
  const roles = JSON.parse(
    fs.readFileSync(path.join(seedDataPath, 'sample-roles.json'), 'utf8')
  )
  
  const policies = JSON.parse(
    fs.readFileSync(path.join(seedDataPath, 'sample-policies.json'), 'utf8')
  )
  
  const users = JSON.parse(
    fs.readFileSync(path.join(seedDataPath, 'sample-users.json'), 'utf8')
  )
  
  const accessRequests = JSON.parse(
    fs.readFileSync(path.join(seedDataPath, 'sample-access-requests.json'), 'utf8')
  )
  ...
  for (const request of data) {
    await prisma.accessRequest.create({
      data: {
        ...
        evaluationResult: request.evaluationMetadata || {},
